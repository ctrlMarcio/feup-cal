...
static Result np_DC(vector<Point> &vp, int left, int right, int numThreads) {
	...
	Result resleft, resright, res;
	if (numThreads > 1) {
		std::thread t([&]() { resleft = np_DC(vp, left, (left + right) / 2, numThreads / 2); });
		resright = np_DC(vp, (left + right) / 2 + 1, right, numThreads / 2);
		t.join();
	} else {
		resleft = np_DC(vp, left, (left + right) / 2, numThreads / 2);
	...
	int i = left;
	for (; vp[i].x < medium - res.dmin; i++);
	left = i;

	for (; i < vp.size() && vp[i].x < medium + res.dmin; i++);
	right = i;
